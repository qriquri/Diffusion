...
def p_losses(denoise_model: nn.Module, x_start: torch.Tensor, t: torch.Tensor, noise: Optional[torch.Tensor] = None):
    ...
    sqrt_alphas_cumprod = torch.sqrt(alphas_cumprod)
    sqrt_one_minus_alphas_cumpord = torch.sqrt(1.0 - alphas_cumprod)
    x_noisy = q_sample(x_start, t, sqrt_alphas_cumprod, sqrt_one_minus_alphas_cumpord, noise=noise)
...
x_start: torch.Tensor = transform(image)
x_start.unsqueeze_(0)
...
for i in range(100):
    t = torch.tensor([i+1], dtype=torch.int64, device=x_start.device) # 1から始まる
    noisy_image = compute_noisy_image(x_start, t)
    noisy_image = reverse_transform(noisy_image[0].squeeze())
    noisy_image.save(f'./noisy_image_{t.item()}.png')
...
def p_sample(model:nn.Module, x:torch.Tensor, t: torch.Tensor, t_index):
    ...
    model_mean = sqrt_recip_aplhas_t * (x - betas_t * model(x, t) / sqrt_one_minus_alphas_cumpord_t)
...
def sample(model: nn.Module, time_steps, image_size, batch_size=16, channels=3):
    return p_sample_loop(model, shape=(batch_size, channels, image_size, image_size), time_steps=time_steps)
